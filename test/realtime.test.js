...
test("phòng nhận tối đa 56 người, 7 người mỗi đội và chặn người thứ 57", async (t) => {
  ...
  const joined = await Promise.all(
    clients.slice(0, 56).map((client, index) => emitAck(client, "player:join", {
      ...
      teamId: `team-${TEAM_NUMBERS[Math.floor(index / 7)]}`,
    })),
  );
...
test("hai người cùng đội giữ ví riêng và không thể dùng đồng thời cùng token", async (t) => {
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  const url = `http://127.0.0.1:${server.address().port}`;
  const host = createClient(url, { transports: ["websocket"] });
  const first = createClient(url, { transports: ["websocket"] });
  ...
  t.after(async () => {
    for (const client of [host, first, second, duplicate]) client.close();
    rooms.clear();
    await new Promise((resolve) => server.close(resolve));
  });
  await Promise.all([host, first, second, duplicate].map(connect));
  const created = await emitAck(host, "host:create");
  const joinedFirst = await emitAck(first, "player:join", { code: created.code, nickname: "An", teamId: "team-1" });
  const joinedSecond = await emitAck(second, "player:join", { code: created.code, nickname: "Bình", teamId: "team-1" });
  assert.notEqual(joinedFirst.playerId, joinedSecond.playerId);
  const blocked = await emitAck(duplicate, "player:join", {
    code: created.code,
    nickname: "Bản sao",
    playerToken: joinedFirst.playerToken,
  });
  assert.equal(blocked.ok, false);
  assert.match(blocked.message, /tab|thiết bị khác/i);

  const firstPlaying = waitForState(first, (state) => state.status === "playing");
  const secondPlaying = waitForState(second, (state) => state.status === "playing");
  await emitAck(host, "host:start", { code: created.code, hostToken: created.hostToken });
  await Promise.all([firstPlaying, secondPlaying]);
  const secondAfterTrade = waitForState(second, (state) => state.game?.tradeTape[0]?.playerName === "An");
  await emitAck(first, "player:trade", { code: created.code, playerToken: joinedFirst.playerToken, symbol: "VNE", side: "buy", quantity: 10 });
  const secondState = await secondAfterTrade;
  assert.equal(secondState.game.portfolio.accountId, joinedSecond.playerId);
  assert.equal(secondState.game.portfolio.holdings.VNE, 0);
});

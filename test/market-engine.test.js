...
test("phiên 15 phút luôn có ít nhất hai tin khẩn nhưng không bao giờ vượt quá ba", () => {
  const guaranteed = setup(1, { durationMs: 15 * 60_000 }).game;
  for (let round = 0; round < 10; round += 1) triggerEvents(guaranteed, 2_000 + round * EVENT_INTERVAL_MS, () => 0.5);
  assert.equal(guaranteed.urgentEventCount, 2);

  const lucky = setup(1, { durationMs: 15 * 60_000 }).game;
  for (let round = 0; round < 10; round += 1) triggerEvents(lucky, 2_000 + round * EVENT_INTERVAL_MS, () => 0);
  assert.equal(lucky.urgentEventCount, 3);
  assert.ok(lucky.activeEvents.length <= 3);
});

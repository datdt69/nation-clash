const HAND_SIZE=6,MAX_ROUNDS=8,TURN_MS=30000,REVEAL_MS=6500;
const TARGETS=[
...
function makeDeck(){let n=0,deck=[];for(const type of Object.keys(CARD_TYPES)){const copies=type==="balance"?4:8;for(let i=0;i<copies;i++)deck.push({id:`card-${++n}-${Math.random().toString(36).slice(2,7)}`,type,...CARD_TYPES[type]})}return shuffle(deck)}
function draw(game,count){if(game.deck.length<count)game.deck.push(...makeDeck());return game.deck.splice(0,count)}
...
function createGame(teams,{mode="socialist",now=Date.now()}={}){
 ...
 const game={mode,seats,economies,deck,round:1,maxRounds:MAX_ROUNDS,target:TARGETS[0],turnIndex:0,phase:"turn",pile:[],history:[],reveal:null,turnEndsAt:now+TURN_MS,startedAt:now};
...
function play(game,teamId,cardIds,now=Date.now()){
 ...
 e.hand=e.hand.filter(c=>!ids.includes(c.id));const p={id:`play-${Date.now()}-${Math.random()}`,teamId,count:cards.length,cards,targetId:game.target.id,at:now};game.pile.push(p);game.history.unshift({type:"play",teamId,count:cards.length,target:game.target.name});game.history=game.history.slice(0,10);game.turnIndex=(game.turnIndex+1)%game.seats.length;game.turnEndsAt=now+TURN_MS;return{ok:true};
}
...
function nextRound(game,now=Date.now()){
 game.round++;if(game.round>game.maxRounds){game.phase="finished";return{finished:true}}const loserId=game.reveal?.loserId;game.pile=[];game.reveal=null;game.target=TARGETS[(game.round-1)%TARGETS.length];game.turnIndex=Math.max(0,game.seats.indexOf(loserId));deal(game);game.phase="turn";game.turnEndsAt=now+TURN_MS;return{finished:false}
}
...
function publicState(game,viewerTeamId=null){
 ...
 return{mode:game.mode,seats:game.seats,economies,round:game.round,maxRounds:game.maxRounds,target:game.target,turnTeamId:currentId(game),phase:game.phase,pile:game.pile.map(p=>({id:p.id,teamId:p.teamId,count:p.count,targetId:p.targetId})),history:game.history,reveal:game.reveal,turnEndsAt:game.turnEndsAt,hand:viewerTeamId?game.economies[viewerTeamId]?.hand||[]:null};
}
module.exports={HAND_SIZE,MAX_ROUNDS,TURN_MS,TARGETS,CARD_TYPES,CRISES,createGame,currentId,play,challenge,tick,winner,setMode,publicState,score,effect};
